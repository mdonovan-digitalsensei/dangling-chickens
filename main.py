...
def bio():
    ...
    print(
        f"You are on level {dungeon_floor_level} in the {dungeon_floor_list[dungeon_floor_level][1]}"
    )
...
while True:
    ...
    if new_floor_load:
        ...
        create_dungeon_floor_monster_list(
            dungeon_floor_list[dungeon_floor_level][3], entities
        )
    ...
    is_in_combat, combat_with_monster = in_combat(
        nobody.return_x(), nobody.return_y(), entities.return_list()
    )
    ...
    if is_in_combat:
        ...
        if action == "q":
            ...
            print(
                f"You have healed, current health {nobody.return_health()} of {nobody.return_my_max_health()}"
            )
